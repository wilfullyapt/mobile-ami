...
class DeviceButtons:
    ...
    Machine button (GPIO 17):
      press  → display.wake()             turn on for 15 s (or reset timer if already on)
    ...
    def __init__(self, config, display, network, core):
        pins = config.get("device", {}).get("button_pins", config.get("button_pins", {}))
        ...
        self.machine_btn.when_pressed = display.wake
